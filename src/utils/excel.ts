  ...
  try {
    ...
    worksheet.eachRow((row, index) => {
      ...
      for (let i = 1; i < cells; i++) {
        ...
        getRowValue(workbook, column, mediaIndex, i, value, meta);
      }
      rows.push(meta);
      mediaIndex++;
    });
...
const getRowValue = (workbook, column, mediaIndex, cellIndex, value, row) => {
  try {
    ...
    switch (descriptor.type) {
      case 'Image':
        try {
          if (mediaIndex < workbook.media.length) {
            const image = workbook.media[mediaIndex];
            const filePath = join(
              descriptor.baseUri,
              Utils.randomString(12) + '.jpg',
            );
      ...
      case 'Date':
        _.set(
          row,
          descriptor.key,
          value ? dayjs(value).format(descriptor.format || 'X') : '',
        );
        break;
      case 'Enum':
        _.set(row, descriptor.key, getEnumValue(descriptor.enum, value));
        break;
      default:
...
export const exportExcel = async (columns, rows, sheetName, style = { row: { height: 32 } }) => {
  try {
    ...
    for (let index = 0; index < rows.length; index++) {
      ...
      const data = setRowValues(columns, row, index, sheet, workbook);
      sheet.addRow(data);
 ...
 * @param {*} workbook 工作簿
 */
const setRowValues = (columns, row, rowIndex, sheet, workbook, style?) => {
  const rowData = [];
